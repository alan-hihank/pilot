...
func (s *StateStore) migrate() error {
	migrations := []string{
		...
		`ALTER TABLE autopilot_pr_state ADD COLUMN source_adapter TEXT DEFAULT ''`,
		`ALTER TABLE autopilot_pr_state ADD COLUMN source_issue_key TEXT DEFAULT ''`,
	}
...
func (s *StateStore) SavePRState(pr *PRState) error {
	_, err := s.db.Exec(`
		INSERT INTO autopilot_pr_state (
			...
			source_adapter, source_issue_key
	...
	`,
		...
		pr.SourceAdapter, pr.SourceIssueKey,
	)
	return err
}
...
func (s *StateStore) GetPRState(prNumber int) (*PRState, error) {
	row := s.db.QueryRow(`
		SELECT pr_number, pr_url, issue_number, branch_name, head_sha,
			...
			COALESCE(source_adapter, ''), COALESCE(source_issue_key, '')
